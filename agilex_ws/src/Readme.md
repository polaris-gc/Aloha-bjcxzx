...
## 启动雷达
```
roslaunch tracer_bringup open_lidar.launch
```
## 启动gmapping建图
```
roslaunch tracer_bringup gmapping.launch
```
用手柄操控小车四处走动建图。

## 保存地图
```
$cd ~/agilex_ws/src/tracer_ros/tracer_bringup/maps
...
(二选一)
```
$#rosrun bunker_bringup bringup_can2usb.bash(不推荐)
```
推荐下面（能使能机械臂和底盘）
...
## 启动导航
```
roslaunch tracer_bringup navigation_4wd.launch 
